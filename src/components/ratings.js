...
class Ratings extends Component {
  ...
  renderStars(maxRating, rating, fullStar = "../Images/StarImage.jpg", blankStar = "../Images/BlankStar.jpg"){
    let results = [];
    for (let i = 1; i <= maxRating; i++) {
      results.push( <img src={(i<=rating)?fullStar:blankStar} alt="Star Image"
          width="40" height="40" value="one" key={i}
          onClick={() => this.props.starSelected(i, this.props.ratingVolume)}/>);
    }
    return results;
  }
  ...
  render() {
    return (
      <div>
        {this.renderStars(this.props.maxRating, this.props.rating)}
      </div>
    );
  }
}
